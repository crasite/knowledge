...
export default function main(sources: Sources): Sinks {
    ...
    const submitField = isolate(InputField)({DOM,props:submitProps})
    ...
    const result$ = O.combineLatest(questionFieldValue,answerField.value)

    const view$ = O.combineLatest(questionField.DOM,addField.DOM,removeField.DOM,submitField.DOM,answerField.DOM).map(doms => p(doms))
    return {
        DOM: view$,
    }
}
